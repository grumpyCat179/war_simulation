...
class TeamCounters:
    ...
    def clone(self) -> "TeamCounters":
        return TeamCounters(self.score, self.kills, self.deaths, self.dmg_dealt, self.dmg_taken)
...
class SimulationStats:
    ...
    Use with 20-tick windowed PPO (team-delta rewards).
    """
    ...
    def add_damage_dealt(self, team: str, amount: float) -> None:
        ...
        t.score += float(amount) * config.TEAM_DMG_DEALT_REWARD

    def add_damage_taken(self, team: str, amount: float) -> None:
        t = self._team(team)
        t.dmg_taken += float(amount)
        t.score += float(amount) * config.TEAM_DMG_TAKEN_PENALTY

    def add_kill(self, team: str, count: int = 1) -> None:
        t = self._team(team)
        t.kills += int(count)
        t.score += float(count) * config.TEAM_KILL_REWARD

    def add_death(self, team: str, count: int = 1) -> None:
        t = self._team(team)
        t.deaths += int(count)
        t.score += float(count) * config.TEAM_DEATH_PENALTY
